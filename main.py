...
def play_audio(file=DETECT_DING, offset=0, overRideVolume=0):
    ...
    GPIO.output(plb_light, GPIO.HIGH)
    i = vlc.Instance('--aout=alsa', '--alsa-audio-device=plughw:0,0')  # , '--alsa-audio-device=mono', '--file-logging', '--logfile=vlc-log.txt')
    m = i.media_new(file)
...
def detect_button(channel):
    global button_pressed, audioplaying, p
    ...
    while (GPIO.input(button) == 0):
        ...
        if time.time() - buttonPress > 10:  # pressing button for 10 seconds triggers a system halt
            play_audio(path + 'resources/' + 'alexahalt.mp3')
...
def silence_listener(throwaway_frames = VAD_THROWAWAY_FRAMES):
...
def start(triggerword=False):
    ...
    while True:
        ...
        #         record_audio = True
            ...
            #         start = time.time()
            #         record_audio = True
            #         play_audio(path + 'resources/' + 'alexayes.mp3', 0)
            #     elif button_pressed:
            #         if audioplaying: p.stop()
            #         record_audio = True
            #
        ...
        if debug == False: os.system("rm /home/pi/AlexaPi/tmpcontent/*") # root/AlexaPi/tmpcontent/*")
            #
        if debug: print "Starting to listen..."
        if triggerword: silence_listener()
	...
	redetect.start(interrupt_check=interrupt_callback, sleep_time=0.03)
        if debug: print('Stage 3')
        if audioplaying: 
            p.stop()
            if debug: print('Stopping previous audio')
            ##############################################################################
...
def setup():
    ...
    for x in range(0, 5):
        ...
        time.sleep(.1)
        GPIO.output(plb_light, GPIO.LOW)
    if (silent == False): play_audio(path + 'resources/' + 'hello.mp3')
    GPIO.add_event_detect(button, GPIO.FALLING, callback=detect_button, bouncetime=100)  # threaded detection of button press
